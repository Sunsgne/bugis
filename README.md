...
Demo 栈与生产 Compose 对齐，使用 **PostgreSQL 17**（非 SQLite），并附带 **Prometheus / Grafana**（仅 Docker 内网，不对外映射端口）。部署地址与登录凭据通过环境变量 / 仓库 Secrets 配置，不在仓库内硬编码。
