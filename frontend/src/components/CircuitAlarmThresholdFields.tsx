    ...
    <>
      ...
      <Row gutter={16}>
        {qosOn && (
          <>
            <Col span={12}>
              <Form.Item name="alarm_latency_ms" label="时延告警 (ms)" extra={latencyHint}>
                <InputNumber min={0} max={10000} style={{ width: "100%" }} placeholder="平台默认" />
              </Form.Item>
            </Col>
            <Col span={12}>
              <Form.Item name="alarm_packet_loss_pct" label="丢包告警 (%)" extra={lossHint}>
                <InputNumber min={0} max={100} step={0.1} style={{ width: "100%" }} placeholder="平台默认" />
              </Form.Item>
            </Col>
          </>
        )}
        <Col span={12}>
          <Form.Item name="alarm_utilization_pct" label="峰值利用率告警 (%)" extra={utilHint}>
            <InputNumber min={0} max={100} style={{ width: "100%" }} placeholder="平台默认" />
          </Form.Item>
        </Col>
        <Col span={12}>
          <Form.Item name="alarm_health_score_min" label="健康分下限" extra={healthHint}>
            <InputNumber min={0} max={100} style={{ width: "100%" }} placeholder="平台默认" />
          </Form.Item>
        </Col>
      </Row>
