...
import { SaveOutlined, ExperimentOutlined, SettingOutlined } from "@ant-design/icons";
...
export default function SnmpSettingsPage() {
  ...
  useEffect(() => {
    load();
    api.get<Device[]>("/devices").then(({ data }) => setDevices(data));
  }, []);
    ...
    >
      <Alert
        ...
        description={
          <Paragraph style={{ marginBottom: 0 }}>
            设备管理中的「SNMP 发现」、检测时的接口采集、以及设备初始化模板中的 SNMP community 均读取此处配置。
            若单台设备在「密码」字段填写了只读 community 且开启「优先使用设备凭证」，将覆盖全局 community。
            也可在 <Link to="/devices">设备管理</Link> 页对单台设备执行发现。
          </Paragraph>
        }
      />

      <Form form={form} layout="vertical" initialValues={{ version: "2c", enabled: true }}>
        <Tabs
          items={[
            {
              key: "basic",
              ...
              children: (
                <>
                  ...
                  <Row gutter={16}>
                    ...
                    <Col xs={12} md={6}>
                      <Form.Item name="max_repetitions" label="GETBULK max-repetitions">
            ...
            },
            {
              ...
              children: (
                <Row gutter={16}>
                  <Col xs={24} md={12}>
                    <Form.Item name="community" label="只读 Community (RO)">
                      <Input placeholder="bugis-ro" />
                    </Form.Item>
                  </Col>
                  <Col xs={24} md={12}>
                    <Form.Item name="write_community" label="读写 Community (RW，可选)">
                      <Input placeholder="仅 Trap / 写操作需要时使用" />
                    </Form.Item>
                  </Col>
                </Row>
              ),
              ...
              children: (
                <>
                  <Row gutter={16}>
                    ...
                    <Col xs={24} md={8}>
                      <Form.Item name="v3_context_name" label="Context Name">
                  ...
                  <Row gutter={16}>
                    ...
                    <Col xs={24} md={6}>
                      <Form.Item
                        name="v3_auth_password"
                        label="认证密码"
                        extra={form.getFieldValue("v3_auth_password_set") ? "已设置，留空则不修改" : undefined}
                      >
                    ...
                    <Col xs={24} md={6}>
                      <Form.Item
                        name="v3_priv_password"
                        label="加密密码"
                        extra={form.getFieldValue("v3_priv_password_set") ? "已设置，留空则不修改" : undefined}
                      >
                        <Input.Password placeholder="留空保持原值" autoComplete="new-password" />
                      </Form.Item>
                    </Col>
              ...
              children: (
                <>
                  <Paragraph type="secondary">
                    控制 IF-MIB _walk 哪些 OID；接口名称过滤支持正则（每行一条）。
                  </Paragraph>
                  <Row gutter={16}>
                    <Col xs={12} md={6}>
                      <Form.Item name="walk_if_descr" label="ifDescr (名称)" valuePropName="checked">
                        <Switch />
                      </Form.Item>
                    </Col>
                    <Col xs={12} md={6}>
                      <Form.Item name="walk_if_alias" label="ifAlias (描述)" valuePropName="checked">
                        <Switch />
                      </Form.Item>
                    </Col>
                    <Col xs={12} md={6}>
                      <Form.Item name="walk_if_high_speed" label="ifHighSpeed (速率)" valuePropName="checked">
                        <Switch />
                      </Form.Item>
                    </Col>
                    <Col xs={12} md={6}>
                  ...
                  <Row gutter={16}>
                    <Col xs={24} md={12}>
                      <Form.Item name="exclude_name_patterns" label="排除接口名 (正则)">
                        <Select mode="tags" placeholder="Loopback, Null0, Vlan-interface" tokenSeparators={[","]} />
                      </Form.Item>
                    </Col>
                    <Col xs={24} md={12}>
                      <Form.Item name="include_name_patterns" label="仅包含 (正则，留空=全部)">
                        <Select mode="tags" placeholder="^GigabitEthernet, ^GE" tokenSeparators={[","]} />
                      </Form.Item>
                    </Col>
                  </Row>
                </>
                ...
                <>
                  <Row gutter={16}>
                    <Col xs={24} md={12}>
                      ...
                      <Text type="secondary">发现后根据端口描述 bw(...) 标签更新骨干链路 capacity</Text>
                    </Col>
                    <Col xs={24} md={12}>
                      <Form.Item name="auto_discover_on_check" label="检测时自动 SNMP 发现" valuePropName="checked">
                        <Switch checkedChildren="开" unCheckedChildren="关" />
                      </Form.Item>
                      <Text type="secondary">设备「检测」可达后自动 walk 接口并入库</Text>
                    </Col>
                  </Row>
                  <Divider />
                  <Form.Item
                    name="baseline_community"
                    label="设备初始化模板 Community"
                    extra="写入 baseline 配置中的 snmp-agent community"
                  >
      ...
      >
        <Form form={testForm} layout="vertical">
          <Form.Item name="device_id" label="选择平台设备">
            <Select
              allowClear
              showSearch
              optionFilterProp="label"
              placeholder="从已纳管设备中选择"
          ...
          <Form.Item name="mgmt_ip" label="或手动输入管理 IP">
            <Input placeholder="10.1.0.11" />
          </Form.Item>
          <Form.Item name="community" label="临时 Community (覆盖全局)">
            <Input placeholder="留空则使用全局/设备凭证" />
          </Form.Item>
        </Form>
        {testResult && (
          <Alert
            ...
            message={
              testResult.ok
                ? `${testResult.target} · ${testResult.interfaces_found} 个接口 · ${testResult.latency_ms} ms`
                : testResult.detail || "探测失败"
            }
            description={
              testResult.sample_interfaces?.length ? (
                <Space wrap size={[4, 4]}>
                  {testResult.sample_interfaces.map((n) => (
                    <Tag key={n}>{n}</Tag>
                  ))}
                </Space>
              ) : undefined
