import { useEffect, useState } from "react";
import { Link } from "react-router-dom";
import {
  Alert,
  Button,
  Card,
  Col,
  Form,
  Input,
  Modal,
  Row,
  Select,
  Table,
  Tag,
  App as AntApp,
  Popconfirm,
  Typography,
} from "antd";
import { PlusOutlined, ShareAltOutlined } from "@ant-design/icons";
import { api } from "../api/client";
import type { Controller } from "../api/types";
import { action, empty, page, toast } from "../constants/uiCopy";
import PageCard from "../components/PageCard";
import { dataTableProps } from "../utils/table";
import { formModalProps } from "../utils/formModal";
import { useTc } from "@/i18n/useTc";
import { translateApiText } from "@/i18n/translateApiText";

const TYPE_LABEL: Record<string, string> = {
  bugis: "Bugis SDN 控制器 (内置)",
  nce_fabric: "华为 NCE-Fabric",
  seerengine: "华三 SeerEngine",
  opendaylight: "OpenDaylight",
  onos: "ONOS",
};
const TYPE_COLOR: Record<string, string> = {
  bugis: "geekblue",
  nce_fabric: "red",
  seerengine: "blue",
  opendaylight: "orange",
  onos: "purple",
};

const EXTERNAL_TYPES = Object.entries(TYPE_LABEL).filter(([value]) => value !== "bugis");

export default function Controllers() {
  const { tc, isEn } = useTc();
  const { message } = AntApp.useApp();
  const [rows, setRows] = useState<Controller[]>([]);
  const [loading, setLoading] = useState(false);
  const [open, setOpen] = useState(false);
  const [form] = Form.useForm();

  async function load() {
    setLoading(true);
    try {
      const { data } = await api.get<Controller[]>("/controllers");
      setRows(data);
    } finally {
      setLoading(false);
    }
  }
  useEffect(() => {
    load();
  }, []);

  async function onCreate() {
    const values = await form.validateFields();
    if (values.type === "bugis") {
      return message.error(tc("内置 Bugis SDN 控制器 · 无需手动纳管"));
    }
    try {
      await api.post("/controllers", values);
      message.success(tc("外部控制器已纳管"));
      setOpen(false);
      form.resetFields();
      load();
    } catch (e: any) {
      message.error(e?.response?.data?.detail || toast.failed);
    }
  }

  async function remove(id: number) {
    const ctrl = rows.find((r) => r.id === id);
    if (ctrl?.type === "bugis") {
      return message.error(tc("内置控制器不可移除"));
    }
    try {
      await api.delete(`/controllers/${id}`);
      message.success(toast.deleted);
      load();
    } catch (e: any) {
      message.error(e?.response?.data?.detail || toast.failed);
    }
  }

  const builtin = rows.find((r) => r.type === "bugis");
  const external = rows.filter((r) => r.type !== "bugis");

  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <Alert
        type="info"
        showIcon
        icon={<ShareAltOutlined />}
        message={tc("内置 vs 北向控制器")}
        description={
          <span>
            <Tag color="geekblue">{tc("Bugis SDN 控制器")}</Tag> {tc("为平台内嵌自研 EVPN 控制平面，启动时自动注册，")}
            <strong>{tc("无需手动纳管")}</strong>。
            {tc("在 Fabric 站点将下发模式设为「控制器托管」并选择内置实例；控制面状态与 EVPN RIB 请前往")}{" "}
            <Link to="/control-plane">{page.controlPlane}</Link>。{tc("下方「纳管外部控制器」用于对接")}
            {tc("华为 NCE-Fabric、华三 SeerEngine 等厂商 / 开源控制器。")}
          </span>
        }
      />

      <PageCard title={tc("内置控制器")}>
        <Table
          rowKey="id"
          loading={loading}
          dataSource={builtin ? [builtin] : []}
          pagination={false}
          locale={{ emptyText: empty.data }}
          {...dataTableProps()}
          columns={[
            { title: tc("名称"), dataIndex: "name", width: "14%", ellipsis: true, render: (n: string) => translateApiText(n, tc, isEn) },
            {
              title: tc("类型"),
              dataIndex: "type",
              width: "18%",
              render: (t) => <Tag color={TYPE_COLOR[t]}>{tc(TYPE_LABEL[t] || t)}</Tag>,
            },
            {
              title: tc("北向地址"),
              dataIndex: "base_url",
              width: "28%",
              ellipsis: true,
              render: (url) => (
                <Typography.Text code copyable>
                  {url}
                </Typography.Text>
              ),
            },
            {
              title: tc("说明"),
              dataIndex: "description",
              width: "28%",
              ellipsis: true,
              render: (d) => translateApiText(d, tc, isEn) || tc("平台内置 · 自动注册"),
            },
            {
              title: tc("操作"),
              width: "12%",
              className: "table-actions",
              render: () => (
                <Link to="/control-plane">
                  <ShareAltOutlined /> {tc("进入控制面")}
                </Link>
              ),
            },
          ]}
        />
      </PageCard>

      <PageCard
        title={tc("北向控制器")}
        extra={
          <Button type="primary" icon={<PlusOutlined />} onClick={() => setOpen(true)}>
            {tc("纳管外部控制器")}
          </Button>
        }
      >
        <Typography.Paragraph type="secondary">
          {tc("控制器托管模式下，平台将业务意图经北向 RESTful 下发至外部控制器，由后者完成底层设备编排。")}
        </Typography.Paragraph>
        <Table
          rowKey="id"
          loading={loading}
          dataSource={external}
          locale={{ emptyText: empty.default }}
          {...dataTableProps()}
          columns={[
            { title: tc("名称"), dataIndex: "name", width: "16%", ellipsis: true, render: (n: string) => translateApiText(n, tc, isEn) },
            {
              title: tc("类型"),
              dataIndex: "type",
              width: "18%",
              render: (t) => <Tag color={TYPE_COLOR[t]}>{tc(TYPE_LABEL[t] || t)}</Tag>,
            },
            { title: tc("北向地址"), dataIndex: "base_url", width: "24%", ellipsis: true },
            { title: tc("账号"), dataIndex: "username", width: "12%", ellipsis: true, render: (v) => v || "—" },
            { title: tc("描述"), dataIndex: "description", width: "22%", ellipsis: true, render: (v) => translateApiText(v, tc, isEn) || "—" },
            {
              title: tc("操作"),
              width: "8%",
              className: "table-actions",
              render: (_, r) => (
                <Popconfirm title={`${action.confirm}${action.delete}？`} onConfirm={() => remove(r.id)}>
                  <Button type="link" size="small" danger>
                    {action.delete}
                  </Button>
                </Popconfirm>
              ),
            },
          ]}
        />
      </PageCard>

      <Modal
        title={tc("纳管外部控制器")}
        open={open}
        onOk={onCreate}
        onCancel={() => setOpen(false)}
        okText={action.create}
        {...formModalProps}
        width={560}
      >
        <Form form={form} layout="vertical" className="app-form" initialValues={{ type: "nce_fabric" }}>
          <Form.Item name="name" label={tc("名称")} rules={[{ required: true }]}>
            <Input placeholder={tc("例如 华为 iMaster NCE-Fabric")} />
          </Form.Item>
          <Form.Item name="type" label={tc("类型")}>
            <Select options={EXTERNAL_TYPES.map(([value, label]) => ({ value, label: tc(label) }))} />
          </Form.Item>
          <Form.Item name="base_url" label={tc("北向地址")} rules={[{ required: true }]}>
            <Input placeholder="https://nce.example.com" />
          </Form.Item>
          <Row gutter={16}>
            <Col xs={24} sm={12}>
              <Form.Item name="username" label={tc("账号")}>
                <Input />
              </Form.Item>
            </Col>
            <Col xs={24} sm={12}>
              <Form.Item name="password" label={tc("密码")}>
                <Input.Password />
              </Form.Item>
            </Col>
          </Row>
          <Form.Item name="description" label={tc("描述")}>
            <Input />
          </Form.Item>
        </Form>
      </Modal>
    </div>
  );
}
