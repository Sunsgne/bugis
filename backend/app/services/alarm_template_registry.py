...
def _default_kinds() -> dict[str, dict[str, str]]:
    return {
        "tunnel_down": {
            ...
            "category": "业务可用性",
            "priority": "P1",
            "title": "【{{priority}}·{{category}}】专线 {{circuit_code}} · {{tenant_name}} 隧道状态异常",
            "detail": "专线 {{circuit_code}}（{{circuit_name}}）· 客户 {{tenant_name}}\nA端 {{endpoint_a_device}} {{endpoint_a_port}} {{endpoint_a_svid}}\nZ端 {{endpoint_z_device}} {{endpoint_z_port}} {{endpoint_z_svid}}\n当前状态：{{status}} · 业务类型 {{service_type}} · 带宽 {{bandwidth_mbps}} Mbps",
        ...
        "circuit_interruption": {
            ...
            "category": "业务可用性",
            "priority": "P1",
            "title": "【{{priority}}·{{category}}】专线 {{circuit_code}} · {{tenant_name}} 业务中断",
            "detail": "专线 {{circuit_code}} · 客户 {{tenant_name}}\n{{endpoint_summary}}\n事件：{{event_detail}}",
        ...
        "sla_loss": {
            ...
            "category": "SLA 质量",
            "priority": "P2",
            "title": "【{{priority}}·{{category}}】专线 {{circuit_code}} · {{tenant_name}} 丢包率越限",
            "detail": "专线 {{circuit_code}} · 客户 {{tenant_name}}\n{{endpoint_summary}}\n实测丢包 {{loss_pct}}% · SLA 阈值 {{threshold_pct}}%",
        ...
        "sla_latency": {
            ...
            "category": "SLA 质量",
            "priority": "P3",
            "title": "【{{priority}}·{{category}}】专线 {{circuit_code}} · {{tenant_name}} 时延越限",
            "detail": "专线 {{circuit_code}} · 客户 {{tenant_name}}\n{{endpoint_summary}}\n实测时延 {{latency_ms}} ms · SLA 阈值 {{threshold_ms}} ms",
        ...
        "utilization": {
            ...
            "category": "容量规划",
            "priority": "P3",
            "title": "【{{priority}}·{{category}}】专线 {{circuit_code}} · {{tenant_name}} 峰值利用率偏高",
            "detail": "专线 {{circuit_code}} · 客户 {{tenant_name}}\n{{endpoint_summary}}\n峰值利用率 {{peak_pct}}% · 告警阈值 {{threshold_pct}}% · 带宽 {{bandwidth_mbps}} Mbps",
        ...
        "health": {
            ...
            "category": "综合健康",
            "priority": "P2",
            "title": "【{{priority}}·{{category}}】专线 {{circuit_code}} · {{tenant_name}} 健康评分偏低",
            "detail": "专线 {{circuit_code}} · 客户 {{tenant_name}}\n{{endpoint_summary}}\n综合健康分 {{score}} · 下限阈值 {{threshold}}",
        ...
        "circuit_flap": {
            ...
            "category": "链路稳定性",
            "priority": "P2",
            "title": "【{{priority}}·{{category}}】专线 {{circuit_code}} · {{tenant_name}} 闪断频繁",
            "detail": "专线 {{circuit_code}} · 客户 {{tenant_name}}\n{{endpoint_summary}}\n近 {{window_min}} 分钟内闪断 {{flaps}} 次 · 超过稳定性阈值",
        ...
        "link_utilization": {
            ...
            "category": "容量规划",
            "priority": "P2",
            "title": "【{{priority}}·{{category}}】骨干 {{link_name}} · {{supplier}} 利用率越限",
            "detail": "链路 {{link_name}} · 供应商 {{supplier}}\n{{endpoint_summary}}\n峰值利用率 {{util_pct}}% · 阈值 {{threshold_pct}}% · 合同带宽 {{cap_display}} · 峰值流量 {{traffic_display}}",
...
_CIRCUIT_VARS = [
    ...
    {"key": "circuit_name", "label": "专线名称"},
    {"key": "tenant_name", "label": "客户/租户名称"},
    {"key": "service_type", "label": "业务类型"},
    {"key": "bandwidth_mbps", "label": "带宽 Mbps"},
    {"key": "endpoint_summary", "label": "A/Z 端摘要"},
    {"key": "endpoint_a_device", "label": "A 端设备"},
    {"key": "endpoint_a_port", "label": "A 端接口"},
    {"key": "endpoint_a_svid", "label": "A 端 S-VID/C-VID"},
    {"key": "endpoint_z_device", "label": "Z 端设备"},
    {"key": "endpoint_z_port", "label": "Z 端接口"},
    {"key": "endpoint_z_svid", "label": "Z 端 S-VID/C-VID"},
]

_LINK_VARS = [
    {"key": "link_name", "label": "链路名称"},
    {"key": "supplier", "label": "供应商"},
    {"key": "device_a_name", "label": "A 端设备"},
    {"key": "device_z_name", "label": "Z 端设备"},
    {"key": "interface_a", "label": "A 端接口"},
    {"key": "interface_z", "label": "Z 端接口"},
    {"key": "link_capacity_mbps", "label": "合同带宽 Mbps"},
    {"key": "endpoint_summary", "label": "A/Z 端摘要"},
    {"key": "util_pct", "label": "利用率 %"},
    {"key": "threshold_pct", "label": "阈值 %"},
    {"key": "cap_display", "label": "合同带宽(展示)"},
    {"key": "traffic_display", "label": "峰值流量(展示)"},
]
...
VARIABLE_CATALOG: dict[str, list[dict[str, str]]] = {
    ...
    "link_utilization": _LINK_VARS,
    "test": [{"key": "product_name", "label": "产品名称"}],
}


def _sample_circuit_context() -> dict[str, Any]:
    return {
        "circuit_code": "CIR-09AAF4",
        "circuit_name": "openai-azure",
        "tenant_name": "OpenAI HK",
        "service_type": "l2vpn_evpn",
        "bandwidth_mbps": 1000,
        "endpoint_summary": (
            "A端 cs-2.tyo7-04 HundredGigE1/0/30 S-VID 19 · "
            "Z端 cs-2.gs-b02 Twenty-FiveGigE1/0/34 S-VID 19"
        ),
        "endpoint_a_device": "cs-2.tyo7-04",
        "endpoint_a_port": "HundredGigE1/0/30",
        "endpoint_a_svid": "S-VID 19",
        "endpoint_z_device": "cs-2.gs-b02",
        "endpoint_z_port": "Twenty-FiveGigE1/0/34",
        "endpoint_z_svid": "S-VID 19",
    }


def _sample_link_context() -> dict[str, Any]:
    return {
        "link_name": "SG-HK-01",
        "supplier": "Equinix",
        "device_a_name": "SG-LEAF-01",
        "device_z_name": "HK-LEAF-01",
        "interface_a": "HundredGigE1/0/49",
        "interface_z": "HundredGigE1/0/49",
        "link_capacity_mbps": 10000,
        "endpoint_summary": "A端 SG-LEAF-01 HundredGigE1/0/49 · Z端 HK-LEAF-01 HundredGigE1/0/49",
        "util_pct": 88.2,
        "threshold_pct": 85,
        "cap_display": "10Gbps",
...
def sample_context(kind: str) -> dict[str, Any]:
    base_circuit = _sample_circuit_context()
    base_link = _sample_link_context()
    samples: dict[str, dict[str, Any]] = {
        "tunnel_down": {**base_circuit, "status": "degraded"},
        "circuit_interruption": {
            **base_circuit,
            "event_detail": "端到端探测判定链路持续中断",
        },
        "sla_loss": {**base_circuit, "loss_pct": 1.25, "threshold_pct": 0.5},
        "sla_latency": {**base_circuit, "latency_ms": 68.2, "threshold_ms": 50},
        "utilization": {**base_circuit, "peak_pct": 92.4, "threshold_pct": 90},
        "health": {**base_circuit, "score": 62.5, "threshold": 70},
        "circuit_flap": {**base_circuit, "flaps": 4, "window_min": 15},
        "link_utilization": base_link,
        "test": {},
    }
